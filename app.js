...
//STEP 1 (~COMPLETED~): ADD EXTRA PROPERTIES TO CHARITY SCHEMA, CREATE CHARITY OBJECT WITH THEM FROM FORM IN POST ROUTE
//STEP 2: ADD MARKER FOR CHARITY FROM FORM TO MAP. ADD INFO PAGE. FOR NOW - NAME + DESC. LATER - ADD A LINK TO CHARITY PAGE. ADD DIFF MARKERS DEPENDING ON CHARITY TYPE?
//STEP 3 1/2: (AUTHENTICATE?) ADD PICTURE FIELDS TO FORM. (TWO DATABASES - APPROVED CHARITY AND CHAIRTY. WE LOOK AT CHARITY AND APPROVE OURSELVES. MOVE CHAIRTY FROM CHARITY DB TO AUTHENTICATED CHARITY OURSELVES. APP.GET GOES THROUGH APPROVED DBS AND SENDS THEM TO MAP.EJS.)
//STEP 3: CREATE TABS ON THE BOTTOM OF THE APP AND BEGIN CREATING OTHER PAGES -OR- CREATE LOGIN
...
function(req, res) {
  ...
  if (user.organization !== undefined) {
    // res.redirect("/place/" + user.organization)
...
app.get("/", function(req, res){ 
  ...
  Charity.find({}, function(err, foundItems){
    ...
    foundItems.forEach(function(item){
      charities.push(item)
      // res.render(__dirname + '/public/map.ejs', {data: foundItems});
    })
    if (req.user !== undefined) {
      res.render(__dirname + '/public/map.ejs', {data: charities, user: req.user.organization})
    } else {
      res.render(__dirname + '/public/map.ejs', {data: charities, user: "not specified"})
    }
...
app.post("/location", function(req, res){
    ...
    axios.get("https://maps.googleapis.com/maps/api/geocode/json", {
      params: {
        address: location,
        key: "AIzaSyBfj0GmRZHpGOZnEgWddZVwd4jNjxQpLEo"
      }
    })
  ...
  req.login(user, function(err){
      if (err) {
          ...
          passport.authenticate("local")(req, res, function(){
              ...
              // res.redirect("/place/" + req.user.organization._id);
              res.redirect("/");
          })
      }
  })
...
app.post("/trtr/:placeId", function(req, res){
  ...
  const ein = req.body.ein;
    ...
    // https://api.data.charitynavigator.org/v2/Organizations/411867244?app_id=9d0d0325&app_key=99db8a2d89f99d28b496906f9d1e6e14
